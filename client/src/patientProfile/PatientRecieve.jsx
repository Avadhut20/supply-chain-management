...
import React, { useEffect, useState } from "react";
import Web3 from "web3";
import contractJson from "../../../blockchain/build/contracts/MedicineTransactionManager.json"; // Adjust path as needed
const contractABI = contractJson.abi; // Adjust path
const contractAddress = "0xB65CF62C53680759767D2f798f5D2436bdfaFEda"; // Replace with actual deployed address
...
const PatientRecieve = () => {
  ...
  useEffect(() => {
    const loadWeb3AndContract = async () => {
      if (window.ethereum) {
        ...
        const orderCount = await instance.methods.orderCounter().call();

        const allOrders = [];
        for (let i = 0; i < orderCount; i++) {
          const order = await instance.methods.orders(i).call();
          const status = await instance.methods.getOrderStatus(i).call();

          if (
            order.patient.toLowerCase() === accounts[0].toLowerCase() &&
            parseInt(status) === 3 // DeliveredToDealer
          ) {
            allOrders.push({
              id: i,
              medicineName: `Medicine ${i + 1}`, // Placeholder name
  ...
  const handleReceive = async (order) => {
    ...
    try {
      await contract.methods.patientReceived(order.id).send({ from: account });
